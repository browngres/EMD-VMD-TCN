...
# including Australia, British, Canada, Switzerland, China, Japan, New Zealand and Singapore ranging from 1990 to 2016.
# task: predict multi-column daily exchange rate from history

folds, enc = get_xy_kfolds(split_index=[0.85])
mse_list = []
...
if __name__ == '__main__':
    ...
    for train_x, train_y, test_x, test_y in folds:
        model = compiled_tcn(return_sequences=False,
                             num_feat=test_x.shape[1],  # 8
                             nb_filters=24,
                             num_classes=0,
                             kernel_size=8,
                             dilations=[2 ** i for i in range(9)],
                             nb_stacks=1,
                             max_len=test_x.shape[0],   #1000
        ...
        print(f"train_set_size:{train_x.shape[0]}")   #  0.85*7588=
